...
public class buku {
    private Stack<data> dataPerpus;

    public static void main(String[] args) throws Exception {
        buku Screen = new buku();
        Screen.inputData();
        Screen.cetakData();
    }

    public void inputData() {
        dataPerpus = new Stack<data>();
        String jawaban;
        int number = 1;
        do {
            ...
            data perpus = new data(judul, pengarang, penerbit, tahun, kategori);
    ...
    public Void cetakData() {
        ...
        for (int i = 0; i < dataPerpus.size(); i++) {
            System.out.println((i + 1) + "\t" + dataPerpus.get(i).judul + "\t" + dataPerpus.get(i).pengarang + "\t"
                    + dataPerpus.get(i).penerbit + "\t" + dataPerpus.get(i).tahun + "\t\t"
                    + dataPerpus.get(i).kategori);
